...
export default function PembelianForm({
    ...
    useEffect(() => {
        ...
        const loadPembelianData = async () => {
            try {
                ...
                const formData = {
                    ...
                    warehouse_id: Number(data.warehouse_id), // Ensure it's a number
                    customer_id: String(data.customer_id), // Ensure it's a string
                    top_id: Number(data.top_id), // Ensure it's a number
                    ...
                    items: data.pembelian_items.map((item) => {
                        ...
                        return {
                            ...
                            price_before_tax: up / (1 + ((item?.tax_percentage || 0) / 100)),
                        };
                    }),
                    attachments: [],
                };

                console.log("[PembelianForm] Form data to reset:", formData);

                setTimeout(() => {
                    form.reset(formData);
                    console.log("[PembelianForm] Form reset complete");
                }, 100);
        ...
        loadPembelianData();
    }, [isEditMode, pembelianId]);
    const watchedItems = form.watch("items");
    const watchedDiscount = Number(form.watch("discount") || 0);
    const watchedAdditionalDiscount = Number(form.watch("additional_discount") || 0);
    const watchedExpense = Number(form.watch("expense") || 0);

    const subTotalBeforeTax = watchedItems.reduce((sum, item) =>
        sum + Number(item.qty || 0) * Number(item.price_before_tax || 0), 0);

    const subTotalAfterTax = watchedItems.reduce((sum, item) =>
        sum + Number(item.qty || 0) * Number(item.unit_price || 0), 0);

    const totalTax = subTotalAfterTax - subTotalBeforeTax;

    const discountAmount = (subTotalBeforeTax * watchedDiscount) / 100;
    const grandTotal =
        (subTotalBeforeTax - discountAmount - watchedAdditionalDiscount) +
        totalTax +
        watchedExpense;
    ...
    const handleSubmit = async (
        ...
        try {
            ...
            // DEBUG: Log all the key variables
            console.log("=== SUBMIT DEBUG INFO ===");
            console.log("mode prop:", mode);
            console.log("isEditMode:", isEditMode);
            console.log("pembelianId prop:", pembelianId);
            console.log("typeof pembelianId:", typeof pembelianId);
            console.log("pembelianId truthy:", !!pembelianId);
            console.log(
                "Edit condition (isEditMode && pembelianId):",
                isEditMode && pembelianId
            );
    ...
    const handleDeleteAttachment = async (attachmentId: number) => {
        try {
            ...
            if (isEditMode && pembelianId) {
                await handleExistingAttachments(pembelianId);
            }
        } catch (error) {
                                                        ...
                                                        render={({field}) => (
                                                            <Input
                                                                ...
                                                                onChange={(e) => {
                                                                    ...
                                                                    field.onChange(val);
                                                                    const before = form.getValues(`items.${index}.price_before_tax`) ?? 0;
                                                                    const newUnit = before + (before * val) / 100;
                                                                    form.setValue(`items.${index}.unit_price`, newUnit, {
                                                                        shouldDirty: true,
                                                                        shouldValidate: true,
                                                                    });
                                                                }}
