    ...
    useEffect(() => {
        ...
        const loadPembelianData = async () => {
            try {
                ...
                const data = await pembelianService.getPembelianById(Number(pembelianId));
                ...
                setSelectedItems(
                    data.pembelian_items.map((item: any) => ({
                        id: item.item_id,
                        name: item.item_name,
                        price: Number(item.unit_price) / (1 + (item.tax_percentage ?? 10) / 100),
                    } as Item))
                );
