import {pembelianService, StatusPembelianEnum} from "@/services/pembelianService";
...
const ReferenceSelectionDialog = ({
    ...
    const handleSelect = (reference: any) => {
        ...
        const totalOutstanding = reference.total_price - (reference.total_paid || 0) - (reference.total_return || 0);

        const selectedRef = {
            id: reference.id,
            no_reference: referenceType === "PEMBELIAN" ? reference.no_pembelian : reference.no_penjualan,
                        ...
                        <Table>
                            ...
                            <TableBody>
                                {references.map((ref) => (
                                    <TableRow key={ref.id}>
                                        <TableCell>
                                            {referenceType === "PEMBELIAN" ? ref.no_pembelian : ref.no_penjualan}
                                        </TableCell>
...
// Simplified selected references table component
    ...
    if (selectedReferences.length === 0) {
        return (
            <div className="text-center py-8 text-muted-foreground">
                No {referenceType === "PEMBELIAN" ? "Pembelian" : "Penjualan"} references added yet
            </div>
        );
    }
...
export {ReferenceSelectionDialog, SelectedReferencesTable, type SelectedReference};