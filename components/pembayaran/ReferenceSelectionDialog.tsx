...
const ReferenceSelectionDialog = ({
  ...
  const loadReferences = async (targetPage: number, searchTerm: string) => {
    ...
    console.log(
      `Loading references - Page: ${targetPage}, Search: "${searchTerm}"`
    );
    setLoading(true);
    try {
  ...
  return (
    <div className={`fixed inset-0 z-50 ${open ? "block" : "hidden"}`}>
      ...
      <div className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 bg-white rounded-lg shadow-lg p-6 w-full max-w-2xl max-h-[80vh] overflow-auto">
