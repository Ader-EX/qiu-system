...
// 2. Add loading state to prevent premature rendering
    ...
    // Get the correct reference ID based on type
    ...
    useEffect(() => {
        ...
        if (mode === "edit" && isDataLoaded && initialDataSet && preloadValues.reference_type) {
            const referenceTypeChanged = watchedReferenceType !== preloadValues.reference_type;

            if (watchedReferenceType === "PEMBELIAN") {
                const vendorChanged = watchedVendorId !== preloadValues.vendor_id;

                if (vendorChanged) {
                    console.log("Vendor changed:", watchedVendorId, "->", preloadValues.vendor_id);
                    setSelectedReferences([]);
            ...
            if (watchedReferenceType === "PENJUALAN") {
                ...
                if (customerChanged) {
                    console.log("Customer changed:", watchedCustomerId, "->", preloadValues.customer_id);
                    setSelectedReferences([]);
                        ...
                        {watchedReferenceType === "PEMBELIAN" ? (
                            isEditMode || isViewMode ? (
                                <FormField
                                    ...
                                    render={({field}) => (
                                        <FormItem>
                                            <SearchableSelect
                                                key={`vendor-${preloadValues.vendor_id || 'empty'}-${isDataLoaded}`}
                                                label="Vendor *"
                            ...
                            <FormField
                                ...
                                render={({field}) => (
                                    <FormItem>
                                        <SearchableSelect
                                            key={`customer-${preloadValues.customer_id || 'empty'}-${isDataLoaded}`}
                                            label="Customer *"
