...
import { QuickFormSearchableField } from "../form/FormSearchableField";
...
const SalesDropdown = () => {
  ...
  const handleBack = () => {
    ...
    setReportData(null);
    setError(null);
    setCustomerId(undefined); // Add this
    setKodeLambungId(undefined); // Add this
  };
    ...
    return (
      <div>
        <div className="flex flex-col space-y-4">
          ...
          <div className="space-y-2">
            ...
            <SearchableSelect
              ...
              onChange={(value) => {
                ...
                setKodeLambungId(undefined); // Reset kode lambung when customer changes
              }}
          ...
          {/* NEW: Kode Lambung filter - only show when customer is selected */}
          {customerId && (
            <div className="space-y-2">
              <label className="block text-sm font-medium text-gray-700">
                Kode Lambung
              ...
              <SearchableSelect
                key={`kodelambung-${customerId}`} // Force re-render when customer changes
                label=""
                ...
                fetchData={async (search) => {
                  const res = await kodeLambungService.getAll({
                    ...
                    size: 10,
                    search,
                    customer_id: customerId, // Pass customer_id to filter
                  });
                  return {
                    data: res.data,
                    total: res.total,
