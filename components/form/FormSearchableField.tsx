  ...
  pembelian: {
    fetchData: (includeDeleted) => async (search) => {
      return await pembelianService.getAllPembelian({
        page: 0,
        size: 3,
        is_picker_view: !includeDeleted,
        search_key: search,
      });
    },
    fetchById: async (id) => {
      const response = await pembelianService.getById(Number(id));
      return { id: response.id, no_pembelian: response.no_pembelian };
    },

    renderLabel: (item: any) => item.no_pembelian,
    transform: (val: any) => Number(val),
  },
  penjualan: {
    fetchData: (includeDeleted) => async (search) => {
    ...
    fetchById: async (id) => {
      const response = await penjualanService.getById(Number(id));
      return { id: response.id, no_penjualan: response.no_penjualan };
    },
    renderLabel: (item: any) => item.no_penjualan,
    transform: (val: any) => Number(val),
  },
  kodelambung: {
