...
const customerSchema = z.object({
    id: z.string().min(1, "ID Customer is required"),
        ...
        const loadCustomerData = async () => {
            try {
                ...
                if (data.kode_lambung) {
                    kodeLambungArray = data.kode_lambung.split(", ").filter(item => item.trim() !== "");
                            ...
                            render={({field}) => (
                                <FormItem className="md:col-span-2">
                                    <SearchableSelect<TOPUnit>
                                        ...
                                        disabled={isViewMode}
                                        onChange={(val) => {
                                            console.log("Selected currency value from SearchableSelect:", val);
                                            if (val === "" || val === "all" || !val) {
                                            ...
                                            console.log("Form currency_id after onChange:", numericValue);
                                        }}
                                ...
                                <div key={index} className="flex items-center space-x-2">
                                    <div className="flex-1">
                                        <Input
                                            ...
                                            value={item}
                                            disabled={isViewMode}
                                            onChange={(e) => updateKodeLambungItem(index, e.target.value)}
                                        />
                                    </div>
                                    {!isViewMode && (
