...
export default function PenjualanForm({
  ...
  const currencyAmount = Number(form.watch("currency_amount") ?? 1) || 1;

  useEffect(() => {
    const rate = Number(form.getValues("currency_amount") ?? 1) || 1;
    const items = form.getValues("items") ?? [];
    items.forEach((item, idx) => {
      const rmb = Number(item.unit_price_rmb ?? 0);
      const idr = convertRMBToIDR(rmb, rate);
      form.setValue(`items.${idx}.unit_price`, idr, {
        shouldValidate: false,
        shouldDirty: true,
      });
    });
    // no deps on form setters
  }, [currencyAmount]);
  ...
  useEffect(() => {
    ...
    const loadPenjualanData = async () => {
      try {
        ...
        console.log(
          "Loading currency_amount:",
          data.currency_amount,
          "->",
          Number(data.currency_amount)
        );
