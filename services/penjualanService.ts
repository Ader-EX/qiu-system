...
class PenjualanService {
    ...
    async downloadAttachment(penjualanId: string, attachmentId: number): Promise<Blob> {
        console.log(`Attempting to download: ${this.baseUrl}/${penjualanId}/download/${attachmentId}`);

        const response = await fetch(`${this.baseUrl}/${penjualanId}/download/${attachmentId}`, {
            method: "GET",
            headers: this.getAuthHeaders(),
        });
