...
export default function PembelianForm() {
  ...
  const handleSubmit = async (
    ...
    try {
      ...
      const newPembelianId = result.id;
      if (!newPembelianId) throw new Error("Failed to get ID from response.");

      // Step 2: Upload attachments if they exist
      const filesToUpload = data.attachments || [];
      if (filesToUpload.length > 0) {
        toast.loading(`Uploading ${filesToUpload.length} attachments...`, {
          id: "upload-toast",
        });
      ...
      // Step 3: Finalize and show success message
      if (finalize) {
        await pembelianService.finalizePembelian(newPembelianId);
        toast.success(`Pembelian has been finalized!`);
      } else {
        toast.success(`Pembelian saved as draft.`);
      }
